...
class PLEResidencyHints:
    ...
    def __post_init__(self) -> None:
        layer_ids = self.resident_layer_ids
        if not isinstance(layer_ids, tuple):
            raise TypeError("resident_layer_ids must be a tuple")
        if any(type(layer_id) is not int or layer_id < 0 for layer_id in layer_ids):
            raise ValueError("resident layer ids must be nonnegative integers")
        if len(set(layer_ids)) != len(layer_ids):
            raise ValueError("resident layer ids must be unique")

    @property
    def compact_slot_by_layer_id(self) -> dict[int, int]:
        """Map sparse global layer ids to compact storage slots.

        Callers that allocate one transfer slot per participating layer must
        use this map instead of indexing a compact array by the global id.
        """
        return {layer_id: slot for slot, layer_id in enumerate(self.resident_layer_ids)}
