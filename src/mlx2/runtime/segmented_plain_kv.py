...
class SegmentedBatchKVCache:
    ...
    def row_views(self, mask):
        """Yield ``(index, valid, keys, values, row_mask)`` per batch row.

        The appended step's authoritative per-row history and that row's
        slice of the prepared mask, for attention kernels that are not plain
        SDPA over ``(keys, values)`` (MLA reads its latent as both).  Rows
        with nothing to attend yield ``valid == 0`` and ``None`` arrays.
        """
        ...
        for index, (row, valid, base) in enumerate(
            ...
            yield index, valid, keys, values, row_mask

    @property
    def step_width(self):
        return max(self._step_lengths or (), default=0)
