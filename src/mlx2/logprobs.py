...
Design reference: sglang#38604. Byte serialization reuses the exact token-byte
helper owned by the structured-output automaton.
...
"""
    ...
    try:
        ...
        byte_values = [list(token_bytes_value(tokenizer, item)) for item in ids]
    except (AttributeError, TypeError, ValueError):
        # Minimal test/custom tokenizers may expose only vocabulary labels. A
        # production tokenizer supplies decode/added-vocabulary metadata and
        # therefore takes the exact shared byte path above.
        byte_values = [list(str(label).encode("utf-8")) for label in labels]
    ...
    def entry(index):
        ...
        return {
            "id": ids[index],
            "token": labels[index],
            "bytes": byte_values[index],
            "logprob": max(value, -9999.0),
        }
