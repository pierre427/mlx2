...
rm -f $D/done2.txt; echo "claude thinking-budget $(date)" > /tmp/gpu.lock
$PY scripts/qualify_serving.py --preflight-only --output $D/preflight.json > $D/preflight.log 2>&1
$PY -u -m mlx2.server --model ~/mlx-models/North-Mini-Code-1.0-mlx-4bit --host 127.0.0.1 --port 8297 --max-context 32768 --max-lanes 4 --max-inflight 8 --cache-bytes 8589934592 --cache-dir $D/cache --ordinary --qualification-mode > $D/server.log 2>&1 &
SERVER=$!
