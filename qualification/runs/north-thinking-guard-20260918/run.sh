...
rm -f $D/done.txt; echo "claude north guard $(date)" > /tmp/gpu.lock
$PY -u -m mlx2.server --model ~/mlx-models/North-Mini-Code-1.0-mlx-4bit --host 127.0.0.1 --port 8297 --max-context 32768 --max-lanes 16 --max-inflight 32 --cache-bytes 8589934592 --cache-dir $D/cache --ordinary --qualification-mode > $D/server.log 2>&1 &
SERVER=$!
until curl -s -m 2 http://127.0.0.1:8297/health | grep -q '"ok"'; do sleep 3; kill -0 $SERVER 2>/dev/null || break; done
$PY $D/experiment.py http://127.0.0.1:8297 $D/results.json > $D/experiment.log 2>&1
