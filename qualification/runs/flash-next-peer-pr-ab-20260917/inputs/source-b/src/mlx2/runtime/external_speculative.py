...
from .speculative_sampling import RequestRNG, probability, verify_proposals
...
class ExternalDraftBatchGenerator:
    ...
    def __init__(self, model, *, draft_model, binding, completion_batch_size=4,
                 ...
                 reclaim_memory=None, evict_checkpoint=None, **kwargs):
        ...
        self.scheduler_stats = {"external_rounds": 0, "accepted_proposals": 0, "proposed_tokens": 0, "ordinary_rounds": 0, "cancelled": 0, "target_max_width": 0, "draft_max_width": 1, "prefill_rounds": 0, "paired_cache_resumes": 0, "segmented_transactions": 0, "segmented_rollbacks": 0, "draft_fallbacks": 0}
        self._open = False
    ...
    def _target_law(self, lane, logits, history):
    ...
    def _round(self, cohort):
        ...
        transaction = None
        try:
            count = min(self.num_draft, min(l.maximum-l.generated-1 for l in cohort))
            if any(l.ordinary for l in cohort): count = 0
            ...
            if count:
                ...
                for indices in groups.values():
                    ...
                    tokens,q = self.draft.draft_distributions(
                        [l.anchor for l in lanes], self.mx.concatenate([l.tail for l in lanes],axis=0),
                        self.draft.batch_caches([l.draft_cache for l in lanes]), count,
                        [l.rng for l in lanes], [float(l.sampling.get("sampling_temp",0)) for l in lanes])
            ...
            transaction = owner.begin(lengths=[count+1]*len(cohort))
            inputs = [[l.anchor]+d for l,d in zip(cohort,drafts)]
            ...
            results = []
            for row, lane in enumerate(cohort):
                targets = [self._target_law(lane, logits[row,j], lane.history + inputs[row][:j+1]) for j in range(count+1)]
                result = verify_proposals(drafts[row], laws[row], targets, lane.rng)
            ...
            self.scheduler_stats["segmented_rollbacks"] += sum(
                int(consumed < count + 1) for _, _, consumed in results
            )
            ...
            if transaction is not None and not transaction.closed:
                ...
                except BaseException: pass  # full independent lane snapshots below are authoritative
    ...
    def next(self):
        ...
        for lane in ready:
            ...
            groups.setdefault(count,[]).append(lane)
        for count, candidates in groups.items():
            cohort = self._fit_cohort(candidates, count+1)
            if not cohort: continue
            try: self._round(cohort)
            except DraftUnavailable:
                for lane in cohort: lane.ordinary = True
                self.scheduler_stats["draft_fallbacks"] += len(cohort)
                self._round(cohort)
