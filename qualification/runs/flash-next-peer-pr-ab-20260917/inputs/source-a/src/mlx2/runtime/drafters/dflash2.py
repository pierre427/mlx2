...
def _draft_distributions(self, anchors, hidden, cache, proposal_length, rngs, temperatures):
    import numpy as np
    from ..speculative_sampling import softmax
    anchors = mx.array(anchors, dtype=mx.int32).reshape(-1)
    ...
    for position in range(proposal_length):
        ...
        for row in range(batch):
            ...
            token = int(ids[row, column]); selected.append(token)
