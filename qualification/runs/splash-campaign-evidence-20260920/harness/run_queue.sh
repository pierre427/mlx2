...
MDIR="~/mlx-models"
LOCK_DIRS="/tmp/gpu.lock /Users/Shared/mlxuag/gpu.lock"
...
sweep_ports() {  # kill anything of ours still holding a queue port
  ...
  for p in $QUEUE_PORTS; do
    ...
    if [ -n "$pids" ]; then
      note "sweeping stray listener on port $p (pids: $(echo $pids | tr '\n' ' '))"
...
LEASE_LABEL=""
LEASE_POLL="${LEASE_POLL:-45}"   # seconds between politeness polls

lease_peer_alive() {  # dir -> 0 if a LIVE peer owns it
  local d="$1" f="$d/owner.json" pid
  [ -d "$d" ] || return 1
  if [ ! -f "$f" ]; then
    # A directory with no receipt: treat as held (someone mid-acquire).
    return 0
  fi
  pid="$(json_field "$f" pid)"
  [ -n "$pid" ] || return 0
  if kill -0 "$pid" 2>/dev/null; then return 0; fi
  return 1   # receipt present but pid dead => stale
}

lease_describe() {  # dir -> who holds it
...
# Acquire every lock dir, waiting politely for any live peer.  All-or-nothing:
# if the second dir is contended we drop the first and retry, so we can never
# deadlock against a peer that takes them in the same order.
lease_acquire() {  # label
  local label="$1" d got waited=0 announced=""
  ...
  while :; do
    ...
    for d in $LOCK_DIRS; do
      ...
      # stale directory (receipt with a dead pid): reclaim it loudly
      if [ -d "$d" ]; then
        note "lease: reclaiming STALE $d ($(lease_describe "$d"))"
        rm -rf "$d"
      fi
      if mkdir "$d" 2>/dev/null; then
        lease_write_receipt "$d" "$label"
        got="$got $d"
      else
        break   # lost the race between the check and the mkdir
      fi
    done
    # did we get them all?
    local want=0 have=0
    ...
    if [ "$want" -eq "$have" ]; then
      ...
      note "lease: ACQUIRED for $label at $(ts) ->$got"
      return 0
    fi
    for d in $got; do rm -rf "$d"; done     # back off cleanly, never hold a partial lease
    sleep "$LEASE_POLL"; waited=$((waited + LEASE_POLL))
  done
}

# Release must run on every exit path: success, gate failure, timeout, Ctrl-C.
lease_release() {
  local d f
...
# REPAIR (M3 fix 2): /v1/status returns 200 while the model is still loading
# (state="loading"), so the old check raced the model load and the first
# requests got HTTP 503 "model is not ready".  /health is the correct signal:
# it 503s until state="ready".  Whatever url a caller passes, /health on the
# same origin must also be green before we call the server ready.
wait_ready() {  # url [max_seconds]
  local url="$1" max="${2:-900}" waited=0 hbase
  hbase="$(printf '%s' "$url" | sed -E 's#(https?://[^/]+).*#\1#')"
  while [ "$waited" -lt "$max" ]; do
    if curl -sf "$url" >/dev/null 2>&1 && curl -sf "$hbase/health" >/dev/null 2>&1; then
      note "ready: $url (+$hbase/health) after ${waited}s"
      return 0
    fi
  ...
  note "FAIL timed out (${max}s) waiting for $url"; return 1
}
...
resolve_served_id() {  # base_url
  SERVED_ID="$(curl -sf "$1/v1/models" 2>/dev/null \
    | sed -n 's/.*"id"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' | head -1)"
  if [ -z "$SERVED_ID" ]; then
    note "WARN could not resolve served model id from $1/v1/models"
    return 1
  fi
...
J
  start_server "$QUEUE_DIR/logs-02-server.log" --model "$MUSE" --host 127.0.0.1 --port 8295 \
    --max-context 32768 --max-lanes 8 --execution-policy /tmp/muse-dflash2-batched.json --external-draft
  wait_ready http://127.0.0.1:8295/v1/status || { stop_server; return 1; }
...
while [ "$i" -lt "${#J_ID[@]}" ]; do
  ...
  grep -E "lease: (WAITING|ACQUIRED|reclaiming)" "$log" | tail -3
  lease_t0=$(date +%s)
  ...
  grep -E "lease: (RELEASED|NOT releasing)" "$log" | tail -2
  lease_dur=$(( $(date +%s) - lease_t0 ))
  echo "### lease held ${lease_dur}s for $id-$slug" >> "$log"
